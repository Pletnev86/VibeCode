...
class ChatContextManager {
  constructor(projectRoot = null, knowledgeBase = null) {
    ...
    this.chatHistory = [];
    this.maxHistoryLength = 6; // Максимум сообщений в истории (3 пары user/ai для экономии токенов)
  ...
  analyzeProjectStructure() {
    ...
    // Фильтруем только файлы, которые реально существуют в файловой системе
  ...
  /**
   * Получение истории разговора (оптимизированная версия)
   */
  getRecentChatHistory(count = 3) {
    const recent = this.chatHistory.slice(-count * 2); // Берем последние N пар сообщений
    
    if (recent.length === 0) {
      return 'История разговора пуста (это начало диалога)';
    }

    let historyText = 'Последние сообщения (только ключевые моменты):\n';
    recent.forEach((entry, index) => {
      const role = entry.role === 'user' ? 'Пользователь' : 'AI';
      // Ограничиваем длину сообщений для экономии токенов
      const maxLength = entry.role === 'user' ? 150 : 300; // Пользовательские сообщения короче
      const message = entry.message.length > maxLength 
        ? entry.message.substring(0, maxLength) + '...' 
        : entry.message;
      historyText += `${index + 1}. ${role}: ${message}\n`;
    });
  ...
  enhanceChatMessage(userMessage) {
    ...
    const recentHistory = this.getRecentChatHistory(3); // Уменьшено с 5 до 3 для экономии токенов
...
⚠️ ВАЖНО: СОХРАНЯЙ КОНТЕКСТ РАЗРАБОТКИ!
- Помни текущую задачу разработки (что создается/дорабатывается)
- Помни структуру проекта и какие файлы уже созданы
- При каждом ответе учитывай предыдущие сообщения и контекст
- Если пользователь просит доработать что-то - используй информацию из истории

ТВОИ ВОЗМОЖНОСТИ:
1. Сохранение файлов: 
   ⚠️ ВАЖНО: НЕ создавай файлы без явного запроса пользователя!
   - Если пользователь просто здоровается или задает вопрос - НЕ создавай файлы
   - Создавай файлы ТОЛЬКО когда пользователь явно просит: "создай файл", "добавь файл", "напиши код для..."
   - Когда пользователь просит создать/изменить файл, предоставь код в формате:
   \`\`\`src/path/to/file.js
   // код файла
   \`\`\`
   ИЛИ
...
3. История разговора (КОНТЕКСТ РАЗРАБОТКИ):
${recentHistory}

⚠️ ИСПОЛЬЗУЙ ИСТОРИЮ для понимания:
- Что уже было создано/изменено
- Какая текущая задача разработки
- Какие файлы уже существуют и их назначение
- Не повторяй уже сделанную работу

4. Структура проектов VibeCode:
⚠️ ВАЖНО: Все файлы проекта должны создаваться в папке проекта!
- Если проект открыт, все файлы сохраняются в: projects/[имя_проекта]/src/
- Путь к файлу должен начинаться с src/ (например: src/test.txt, src/components/App.js)
- НЕ создавай файлы в корне проекта или вне папки src/
- Пример правильного пути: src/test.txt, src/components/App.jsx, src/styles/main.css

Важные пути:
- Корень проекта VibeCode: ${projectRoot}
- Если проект открыт, файлы проекта: projects/[имя_проекта]/src/
- Исходники проекта: ${path.join(projectRoot, 'src')}
...
5. Формат ответов:
⚠️ КРИТИЧЕСКИ ВАЖНО для путей к файлам:
- ВСЕГДА указывай путь начиная с src/ (например: src/test.txt, НЕ просто test.txt)
- Путь должен быть: src/[имя_файла] или src/[папка]/[имя_файла]
- Используй формат: \`\`\`src/test.txt\nсодержимое\n\`\`\` или \`\`\`javascript\nsrc/components/App.js\nкод\n\`\`\`
- НЕ используй плейсхолдеры типа "filepath" или "path/to/file"
- НЕ создавай файлы без src/ в начале пути
- Каждый файл должен быть в отдельном блоке кода
- Примеры правильных путей:
  ✅ src/test.txt
  ✅ src/components/App.js
  ✅ src/styles/main.css
  ❌ test.txt (НЕПРАВИЛЬНО - нет src/)
  ❌ /src/test.txt (НЕПРАВИЛЬНО - не начинай с /)
...
9. ИНСТРУКЦИИ ПО ФАЙЛОВОЙ ИНФРАСТРУКТУРЕ:
📋 Быстрые шаблоны для создания проектов:
...
- src/components/App.jsx (главный компонент)
- src/components/Header.jsx, Footer.jsx (компоненты)
- src/styles/main.css (стили)

**Electron приложение:**
- src/main.js (главный процесс)
- src/preload.js (preload скрипт)
- src/renderer/index.html (renderer)
- src/renderer/renderer.js (renderer логика)

**Node.js приложение:**
- src/index.js (точка входа)
- src/config/config.js (конфигурация)
- src/routes/api.js (маршруты)
- src/utils/helpers.js (утилиты)

⚠️ ВАЖНО: Все пути должны начинаться с src/!

10. ИНФОРМАЦИЯ О МОДЕЛЯХ AI:
🤖 Модель llama-3-8b-gpt-4o-ru1.0 (локальная):
- Понимает русский и английский языки одинаково хорошо
- НЕ требует перевода запросов на английский
- Отвечай на том же языке, на котором задан вопрос
- Работает с русским и английским текстом без ограничений
