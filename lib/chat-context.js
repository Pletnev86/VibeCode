...
class ChatContextManager {
  ...
  getProjectContext() {
    // Проверяем кеш
    if (this.projectStructure && this.structureCacheTime) {
      const cacheAge = Date.now() - this.structureCacheTime;
      if (cacheAge < this.structureCacheTTL) {
    ...
    try {
      ...
      this.logger.warn('Ошибка анализа структуры проекта', null, error);
